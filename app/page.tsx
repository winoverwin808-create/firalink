...
export default function Home() {
  ...
  const staticPages = [
    { label: "About Firalink Hub", sub: "Purpose, services & how it works", key: "about-page" },
  ...
  function renderWorkCard(w: any) {
    ...
    return (
      <div style={{ ...workCard, boxShadow: isHighlighted ? "0 0 0 3px #8B2FD9" : workCard.boxShadow }} key={w.id}>
        <div style={thumb}>
    ...
    <div style={{ fontFamily: "Inter, sans-serif", color: "#1A1523", background: "#F5F3F9", maxWidth: 430, margin: "0 auto", minHeight: "100vh", paddingBottom: 90, position: "relative" as const }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px 20px 6px" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <div style={iconBtn} onClick={openSearch}>🔍</div>
      ...
      <div ref={setRef("works")} style={{ padding: "20px 20px 4px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <h2 style={{ ...sectionTitle, margin: 0, textTransform: activeFilter ? ("none" as const) : ("none" as const) }}>{worksHeading}</h2>
          {activeFilter ? (
      ...
      <div ref={setRef("about")} style={{ padding: "20px 20px 4px" }}>
        <h2 style={sectionTitle}>About Firalink Hub</h2>
        <div style={{ background: "#fff", border: "1px solid #ECE8F5", borderRadius: 20, padding: 20, boxShadow: "0 10px 24px rgba(107,60,180,0.08)" }}>
          ...
          <div style={{ fontSize: 12.5, fontWeight: 700, color: "#7C3AED", cursor: "pointer" }} onClick={function () { router.push("/about"); }}>Read the full guide</div>
        </div>
      </div>
      ...
      {["graphics", "video", "documents"].map(function (type) {
        ...
        return (
          <div ref={setRef(type)} style={{ padding: "20px 20px 4px" }} key={type}>
            <div style={catHeader} onClick={function () { toggleExpand(type); }}>
              ...
              <div style={{ fontSize: 13, color: "#A6A3B0", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.15s" }}>⌄</div>
            </div>
            {isOpen ? (
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" as const, marginTop: 12 }}>
                {subs.length > 0 ? subs.map(function (cat) {
                  ...
                  return (
                    <div style={isActive ? cardChipActive : cardChip} key={cat.id} onClick={function () { selectSubcategory(cat); }}>{cat.name}</div>
                  );
      ...
      <div ref={setRef("services")} style={{ padding: "20px 20px 4px" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {[{ label: "🎨 Graphics", type: "graphics" }, { label: "🎬 Video", type: "video" }, { label: "📄 Documents", type: "documents" }].map(function (s) {
            return (
              <div key={s.type} style={{ flex: 1, background: "#fff", border: "1px solid #ECE8F5", borderRadius: 16, padding: "16px 8px", textAlign: "center" as const, boxShadow: "0 10px 24px rgba(107,60,180,0.08)", fontSize: 11.5, fontWeight: 700, cursor: "pointer" }} onClick={function () { focusCategorySection(s.type); }}>{s.label}</div>
            );
          })}
        </div>
      </div>

      <div ref={setRef("team")} style={{ padding: "20px 20px 4px" }}>
      ...
      <div ref={setRef("cta")} style={{ margin: "28px 20px 10px", borderRadius: 22, padding: "26px 22px", textAlign: "center" as const, background: "linear-gradient(135deg,#8B2FD9,#5B1FA6)", boxShadow: "0 14px 30px rgba(91,31,166,0.28)" }}>
      ...
      <div style={{ margin: "0 20px 20px", padding: "20px 4px 0", borderTop: "1px solid #E4DFF0" }}>
        <div style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 10 }}>Firalink Hub</div>
        <div style={{ display: "flex", flexWrap: "wrap" as const, gap: "8px 18px", marginBottom: 14 }}>
          <span style={{ fontSize: 12, color: "#6E6B7A", cursor: "pointer" }} onClick={goHome}>Home</span>
          <span style={{ fontSize: 12, color: "#6E6B7A", cursor: "pointer" }} onClick={function () { router.push("/about"); }}>About</span>
          <span style={{ fontSize: 12, color: "#6E6B7A", cursor: "pointer" }} onClick={function () { setActiveNav("home"); setTimeout(function () { scrollToSection("services"); }, 60); }}>Services</span>
          <span style={{ fontSize: 12, color: "#6E6B7A", cursor: "pointer" }} onClick={goToTeamSection}>Team</span>
          <a href="/upload" style={{ fontSize: 12, color: "#6E6B7A", textDecoration: "none" }}>Post a Work</a>
        </div>
        <p style={{ margin: 0, fontSize: 11, color: "#A6A3B0" }}>© {new Date().getFullYear()} Firalink Hub · Built for the Telegram Mini App</p>
      </div>
      ...
      {showNotif ? (
        <div style={overlay} onClick={function () { setShowNotif(false); }}>
          <div style={sheet} onClick={function (e) { e.stopPropagation(); }}>
            ...
            <button style={{ ...btn, marginTop: 14, background: "#F1E9FB", color: "#7C3AED" }} onClick={function () { setShowNotif(false); setShowForm(true); }}>Start a Request</button>
          </div>
        </div>
      ) : null}

      {searchOpen ? (
        <div style={{ position: "fixed" as const, inset: 0, background: "#F5F3F9", zIndex: 60, maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column" as const }}>
          <div style={{ padding: "20px 20px 12px", display: "flex", alignItems: "center", gap: 10, background: "#fff", borderBottom: "1px solid #ECE8F5" }}>
            <div style={{ ...iconBtn, border: "none", background: "#F5F3F9" }} onClick={closeSearch}>←</div>
            <input
              autoFocus
                ...
                {catMatches.length > 0 ? (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {catMatches.map(function (c) {
                      return (
                        <div key={c.id} style={searchResultRow} onClick={function () { goToCategoryResult(c); }}>
                ...
                {workMatches.length > 0 ? (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {workMatches.map(function (w) {
                      return (
                        <div key={w.id} style={searchResultRow} onClick={function () { goToWorkResult(w); }}>
                ...
                {teamMatches.length > 0 ? (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {teamMatches.map(function (m) {
                      return (
                        <div key={m.id} style={searchResultRow} onClick={goToTeamResult}>
                ...
                {pageMatches.length > 0 ? (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {pageMatches.map(function (p) {
                      return (
                        <div key={p.key || p.href} style={searchResultRow} onClick={function () { goToPageResult(p); }}>
                          <div style={searchResultIcon}>📄</div>
                          <div>
